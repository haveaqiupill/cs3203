...
ClauseAnswer CallsEvaluator::evaluateCalls(QueryArg& leftArg,
                                           QueryArg& rightArg,
    ...
    if (leftArgType == QueryArgType::NAME) {
        if (rightArgType == QueryArgType::WILDCARD) {
            ...
            std::vector<PROC_NAME> callees = pkb->getCallees(leftName);
        ...
        if (rightArgType == QueryArgType::WILDCARD) {
            ...
            if (leftSyn == rightSyn) {
                ...
                std::vector<std::pair<PROC_NAME, PROC_NAME>> procPairs = pkb->getCallsPair(
            ...
            std::vector<PROC_NAME> callers = pkb->getCallers(rightName);
            resultTable = EvaluatorUtility::strVecToResultTable(leftDec, callers);
        }
