...
ClauseAnswer FollowsStarEvaluator::evaluateFollowsStar(QueryArg& leftArg,
                                                       QueryArg& rightArg,
    ...
    if (leftArgType == QueryArgType::NUM) {
        if (rightArgType == QueryArgType::WILDCARD) {
            ...
            std::vector<LINE_NUM> childLineNums = pkb->getFollowsStarAfter(leftLineNum,
                                                                           rightSynType);
        ...
        if (rightArgType == QueryArgType::WILDCARD) {
            ...
            std::vector<LINE_NUM> parentLineNums = pkb->getFollowsStarBefore(leftSynType);
            ...
            if (leftSyn == rightSyn) {
                ...
                std::vector<std::pair<LINE_NUM, LINE_NUM>> lnPairs = pkb->
                    getFollowsStarPair(leftSynType, rightSynType);
        ...
        if (rightArgType == QueryArgType::WILDCARD) {
            ...
            std::vector<LINE_NUM> childLineNums = pkb->getFollowsStarAfter(rightSynType);
            resultTable = EvaluatorUtility::lineNumVecToResultTable(rightDec, childLineNums);
        } else if (rightArgType == QueryArgType::NUM) {
            // Follows*(_, 5)
