...
ClauseAnswer AffectsBipEvaluator::evaluateAffectsBip(QueryArg& leftArg,
    QueryArg& rightArg,
    ...
    if (leftArgType == QueryArgType::NUM) {
        if (rightArgType == QueryArgType::WILDCARD) {
            ...
            std::vector<LINE_NUM> linesAft = pkb->getAffectsBipLines(leftLineNum, rightSynType);
        ...
        if (rightArgType == QueryArgType::WILDCARD) {
            ...
            std::vector<LINE_NUM> parentLineNums = pkb->getAffectingBipLines(leftSynType);
            ...
            std::vector<std::pair<LINE_NUM, LINE_NUM>> lnPairs = pkb->getAffectsBipLinesPair(
            ...
            std::vector<LINE_NUM> linesBef = pkb->getAffectingBipLines(leftSynType, rightLineNum);
            resultTable = EvaluatorUtility::lineNumVecToResultTable(leftDec, linesBef);
        }
    } else if (leftArgType == QueryArgType::WILDCARD) {
        if (rightArgType == QueryArgType::WILDCARD) {
            // AffectsBip(_, _)
            return ClauseAnswer(pkb->hasAffectsBipRelationship(), resultTable);
        } else if (rightArgType == QueryArgType::DECLARATION) {
            // AffectsBip(_, ?)
            Declaration rightDec = rightArg.getDeclaration();
            SYNONYM rightSyn = rightDec.getSynonym();
            ...
            std::vector<LINE_NUM> childLineNums = pkb->getAffectsBipLines(rightSynType);
