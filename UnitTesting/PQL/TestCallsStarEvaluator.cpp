...
#include "PQL/Evaluator/CallsStarEvaluator.h"
#include "PKB/PKB.h"
...
namespace UnitTesting {
    ...
    public:
        TEST_METHOD(EvaluateCallsStar_ProcNameDeclaration_Success) {
            // Call*("alpha", p)
            std::string syn = "p";
            ...
            pkb.storeRelationshipCallsStar("alpha", "beta");
            ...
            ClauseAnswer actual = calStar.evaluateCallsStar(leftArg, rightArg, dt);
        ...
        TEST_METHOD(EvaluateCallsStar_DeclarationProcName_Success) {
            // Call*(p, "beta")
            std::string syn = "p";
            std::unordered_map<Declaration, int, dec_hash> synonyms = {
                {
    ...
    private:
        PKB pkb;
        CallsStarEvaluator calStar = CallsStarEvaluator(&pkb);
    };
}  // namespace UnitTesting
