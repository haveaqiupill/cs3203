...
#include "PQL/Evaluator/FollowsStarEvaluator.h"
#include "PKB/PKB.h"
...
namespace UnitTesting {
    ...
    public:
        TEST_METHOD(EvaluateFollowsStar_LineNumWildCard_Success) {
            // Follows*(1, _)
            ...
            ClauseAnswer actual = folStar.evaluateFollowsStar(leftArg, rightArg, dt);

            Assert::AreEqual(expected.getIsClauseTrue(), actual.getIsClauseTrue());
        }
        TEST_METHOD(EvaluateFollowsStar_LineNumDeclaration_Success) {
            // Follows*(1, s)
            std::string syn = "s";
        ...
        TEST_METHOD(EvaluateFollowsStar_DeclarationLineNum_Success) {
            // Follows*(s, 5)
            std::string syn = "s";
            std::unordered_map<Declaration, int, dec_hash> synonyms = {
                {
        ...
        TEST_METHOD(EvaluateFollowsStar_DeclarationWildcard_Success) {
            // Follows*(s, _)
            std::string syn = "s";
            std::unordered_map<Declaration, int, dec_hash> synonyms = {
                {
            ...
            pkb.propagateStmtList(5);
            pkb.storeRelationshipFollows(3, 5);
            pkb.storeRelationshipFollowsStar(3, 5);
            pkb.sortAllDataStructure();
        ...
        TEST_METHOD(EvaluateFollowsStar_WildcardWildcard_Success) {
            // Follows*(_, _)
            ClauseAnswer expected = ClauseAnswer(true, ResultTable());
        ...
        TEST_METHOD(EvaluateFollowsStar_WildcardLineNum_Success) {
            // Follows*(_, 5)
            std::unordered_map<Declaration, int, dec_hash> synonyms = { };
    ...
    private:
        PKB pkb;
        FollowsStarEvaluator folStar = FollowsStarEvaluator(&pkb);
    };
}  // namespace UnitTesting
